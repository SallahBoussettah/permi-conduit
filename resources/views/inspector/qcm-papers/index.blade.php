    ...
    <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <h1 class="text-3xl font-bold text-gray-900">{{ __('QCM Papers') }}</h1>
            ...
            <p class="mt-2 text-sm text-gray-700">{{ __('Create and manage QCM papers for your permit categories.') }}</p>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-md">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <p class="text-sm text-gray-500">
                                                    {{ $paper->permitCategory->name ?? 'No category' }} · {{ $paper->questions()->count() }} questions
                                                </p>
                                            </div>
                                        </div>
                                        <div class="flex items-center space-x-2">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $paper->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ $paper->status ? __('Active') : __('Inactive') }}
                                            </span>
                                            <div class="flex space-x-2">
                                                ...
                                                <form action="{{ route('inspector.qcm-papers.destroy', $paper) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500" onclick="return confirm('{{ __('Are you sure you want to delete this paper?') }}')">
                                                        {{ __('Delete') }}
                                                    </button>
                                                </form>
                                            </div>
                                    ...
                                    <div class="mt-2 sm:flex sm:justify-between">
                                        <div class="sm:flex">
                                            <p class="flex items-center text-sm text-gray-500">
                                                ...
                                                {{ Str::limit($paper->description, 100) ?? __('No description') }}
                                            </p>
                                        </div>
                                        <div class="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                                            ...
                                            <p>
                                                {{ __('Created') }} {{ $paper->created_at->format('M d, Y') }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
            ...
            @else
                <div class="px-4 py-8 text-center">
                    ...
                    <h3 class="mt-2 text-lg font-medium text-gray-900">{{ __('No QCM papers found') }}</h3>
                    <p class="mt-1 text-sm text-gray-500">{{ __('Get started by creating a new QCM paper.') }}</p>
                    <div class="mt-6">
